...
test('geçerli quranQuotes satırları dil bazında normalize edilir', () => {
    const out = normalizeRemoteDailyQuotes({
        quranQuotes: {
            en: [
                ...
                [94, 5, 'With hardship comes ease.', 6],
            ],
    ...
    assert.deepEqual(out.quranQuotes.en[1], [94, 5, 'With hardship comes ease.', 6]);
...
test('mevcut public/daily-quotes.json şemaya uyuyor', async () => {
    ...
    assert.ok(out.quranQuotes, 'quranQuotes eksik');
    assert.ok(out.quranQuotes.en?.length >= 20, 'en quranQuotes eksik');
    assert.ok(out.quranQuotes.ar?.length >= 20, 'ar quranQuotes eksik');
    const multi = out.quranQuotes.en.find((r) => r[0] === 94 && r[1] === 5);
    assert.deepEqual(multi?.slice(0, 2), [94, 5]);
    assert.equal(multi?.[3], 6);
});
