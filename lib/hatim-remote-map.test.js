...
test('üye listesi: ilk bakışta nokta yok, sonra katılan yeşil ayrılan kırmızı', () => {
    ...
    assert.deepEqual(
        sonra.remote.seenMembers,
        [{ uid: 'u1', name: '' }, { uid: 'u2', name: '' }],
        'son bakış taşınır'
    );
    assert.deepEqual(memberDiff(sonra), {
        joined: ['u3'],
        left: [{ uid: 'u2', name: '' }],
        firstLook: false
    });
...
test('üye listesi ve son bakış depoda hayatta kalır', () => {
    ...
    assert.deepEqual(geri.remote.seenMembers, [
        { uid: 'u1', name: '' },
        { uid: 'u2', name: '' }
    ]);
...
test('ayrılan kişi son bakıştaki adıyla anılır', () => {
    // Sunucudan düştüğü için adı başka yerden öğrenilemez; listede
    // "Ali (ayrıldı)" yazabilmenin tek yolu adı son bakışta saklamak.
    const veri = { code: 'K7M2QP', rev: 1, memberCount: 2 };
    const ilk = groupFromRemote(HATIM_ID, veri, new Map(), null, ['u1', 'u2']);
    const bakildi = withSeenMembers(ilk, [
        { uid: 'u1', name: 'Ömer' },
    ...
    const sonra = groupFromRemote(HATIM_ID, veri, null, bakildi, ['u1']);
    const fark = memberDiff(sonra);
    assert.deepEqual(fark.left, [{ uid: 'u2', name: 'Ali' }]);
    assert.deepEqual(fark.joined, []);

    // Liste açılınca ayrılan düşer, bir daha görünmez.
    const tekrar = withSeenMembers(sonra, [{ uid: 'u1', name: 'Ömer' }]);
    assert.deepEqual(memberDiff(tekrar), { joined: [], left: [], firstLook: false });
});
