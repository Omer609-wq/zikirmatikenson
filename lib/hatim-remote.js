 ...
 *
 * Her açılışta 2 okuma: hatim belgesi + kişinin kendi üyelik kaydı. Cüz tablosu
 * (+30) yalnızca rev önbellekle farklıysa okunur (§3).
 *
...
export async function fetchHatim(ctx, hatimId, previous = null) {
    ...
    const { db, uid } = ctx;
    try {
        const [hatimSnap, mineSnap] = await Promise.all([
        ...
        if (!needsJuzRefetch(previous, h.rev)) {
            return { ok: true, group: groupFromRemote(hatimId, h, null, previous), juzFetched: false };
        }
        ...
        return { ok: true, group: groupFromRemote(hatimId, h, byN, previous), juzFetched: true };
    } catch (err) {
        return { ok: false, reason: reasonFromError(err) };
    }
}
