...
test('resolveLibraryId ile libraryId taşımayan zikirler eşlenebilir (varsayılan zikirler)', () => {
    // z_1 (Subhanallah) libraryId taşımaz ama kimliğe göre lib_6'ya eşlenir.
    const CLASSIC = { z_1: 'lib_6', z_3: 'lib_5' };
    const resolve = (z) => z.libraryId || CLASSIC[z.id] || null;
    const zikirs = [
        { id: 'z_1', folderId: 'f_default' }, // Subhanallah, libraryId yok
        { id: 'z_9', libraryId: 'lib_6', folderId: 'f_sabah' } // kullanıcı eklemesi
    ];
    assert.deepEqual(
        getFoldersForLibraryItem('lib_6', zikirs, FOLDERS, resolve).map((f) => f.id),
        ['f_default', 'f_sabah']
    );
    // Resolver verilmezse eski davranış: libraryId taşımayan sayılmaz.
    assert.deepEqual(getFoldersForLibraryItem('lib_6', zikirs, FOLDERS).map((f) => f.id), ['f_sabah']);
});
