...
import { sanitizeHatimGroups } from './hatim-groups.js';
...
/** Kayıtlı zikir adı üst sınırı. Kütüphane okunuşları 348 karaktere kadar
 *  çıkıyor (ör. tesbihat istiğfarının Arapçası); eski 80'lik sınır bunları
 *  kelime ortasından kırpıyordu. */
const ZIKIR_NAME_MAX = 400;
...
export function sanitizeLoadedData(d) {
    ...
    zArr = zArr
        ...
        .map((z, idx) => ({
            ...
            name: coerceString(z.name || 'Zikir', ZIKIR_NAME_MAX) || `Zikir ${idx + 1}`,
            ...
            .map((x) => ({
                ...
                zikirs: Array.isArray(x.zikirs)
                    ? x.zikirs.filter(isPlainObject).slice(0, 2000).map((z) => ({
                        id: coerceId(z.id, 'z'),
                        folderId: coerceId(z.folderId || 'f_default', 'f'),
                        name: coerceString(z.name || 'Zikir', ZIKIR_NAME_MAX),
                        arabic: coerceString(z.arabic || '', 1200),
                        target: clampNumber(z.target, { min: 1, max: 1000000, fallback: 33 }),
                        meaning: coerceString(z.meaning || '', 1600),
                        count: clampNumber(z.count, { min: 0, max: 1000000000, fallback: 0 }),
                ...
                zikir: x.zikir && isPlainObject(x.zikir)
                    ? {
                        ...
                        name: coerceString(x.zikir.name || 'Zikir', ZIKIR_NAME_MAX),
    ...
    const appMetaOut = {
        ...
        // Hatim gruplarında cüz sahipliği bu yerel kimliğe bağlanır.
        memberId: coerceString(meta.memberId || '', 64) || null,
        // Hatimde cüz alırken son kullanılan ad; istem bununla dolu gelir.
        memberName: coerceString(meta.memberName || '', 20) || null,
        // Paylaşımlı hatimde cüz sahibi Firebase uid'sidir; ekran çizimi eşzamanlı
        // olduğu için son bilinen uid saklanır (lib/hatim-session.js).
        hatimUid: coerceString(meta.hatimUid || '', 128) || null
    };
    ...
    return {
        ...
        hatimGroups: sanitizeHatimGroups(safe.hatimGroups)
    };
}
