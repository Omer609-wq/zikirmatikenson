...
function sanitizeStoredZikir(z, idx, orderFallback = idx) {
    return {
        id: coerceId(z.id, 'z'),
...
export function sanitizeLoadedData(d) {
    ...
    zArr = zArr
        ...
        .slice(0, 200000)
        .map((z, idx) => sanitizeStoredZikir(z, idx));
            ...
            .map((x) => ({
                ...
                zikirs: Array.isArray(x.zikirs)
                    ? x.zikirs
                          ...
                          .slice(0, 2000)
                          .map((z, idx) => sanitizeStoredZikir(z, idx, clampNumber(z.order, { min: 0, max: 1e9, fallback: 0 })))
                    : undefined,
                zikir: x.zikir && isPlainObject(x.zikir)
                    ? sanitizeStoredZikir(
                          x.zikir,
                          0,
                          clampNumber(x.zikir.order, { min: 0, max: 1e9, fallback: 0 })
                      )
                    : undefined,
