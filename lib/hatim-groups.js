...
/** Aynı anda üye olunabilecek grup sayısı — her grup açılışta 1 okuma demek. */
export const HATIM_GROUP_LIMIT = 5;
 ...
 * ayırt edilemez. Üç kademe rahat ayrılır ve üst kademeler zaten nadirdir —
 * 4'e çıkmak için dört ayrı hatimde aynı cüz numarasını seçmiş olman gerekir
 * (toplam hatim sınırı beş). Bu yüzden 4 ve üstü tek kademede doyurulur.
 */
export const HATIM_LOAD_STEPS = [1, 3, Infinity];
    ...
    return raw
        ...
        .map((g, idx) => {
            ...
            return {
                ...
                // Kip bilinmiyorsa ortak sayılır: kip alanından önce kurulan
                // gruplar grup akışıyla açılmıştı.
                kind: HATIM_KINDS.has(g.kind) ? g.kind : HATIM_KIND_SHARED,
