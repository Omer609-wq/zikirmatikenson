...
test('ingilizcede 1 tekil, digerleri cogul', () => {
    ...
    assert.equal(tEn('community.cardGroups', { count: 1 }), '1 active group');
...
test('yeni cevrilen arapca sayili metinler', () => {
    ...
    assert.equal(tAr('community.cardGroups', { count: 1 }), 'مجموعة نشطة واحدة');
    assert.equal(tAr('community.cardGroups', { count: 2 }), 'مجموعتان نشطتان');
    assert.equal(tAr('community.cardGroups', { count: 4 }), '4 مجموعات نشطة');
});
