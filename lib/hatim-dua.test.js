...
test('ortak duanın üç katmanı aynı paragraf sayısında', { skip: !SHARED_TR }, () => {
    ...
    assert.equal(say(duaData.shared.translit.tr), arapca, 'okunuş paragraf sayısı tutmuyor');
    assert.equal(say(duaData.shared.meaning.tr), arapca, 'meal paragraf sayısı tutmuyor');
});
