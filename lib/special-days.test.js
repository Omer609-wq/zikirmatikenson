...
/** Diyanet'in listeleri miladi yıla göre; veri kontrolleri de öyle. */
const specialDaysForYear = (list, year) => list.filter((e) => e.date.startsWith(`${year}-`));
const activeId = (now) => {
...
test('uzaktan gelen yil gomulunun o yilinin yerine gecer, digerleri kalir', () => {
    ...
    assert.deepEqual(specialDaysForYear(merged, 2028).map((e) => e.date), ['2028-02-24']);
...
test('ad ve hicri tarih turkce', () => {
    ...
    assert.equal(formatHijriDayMonthTr([29, 9, 1448]), '29 Ramazan');
...
test('liste hicri yila gore: 1448 Hicri Yilbasi ile baslar, Kurban Bayrami ile biter', () => {
    const y1448 = specialDaysForHijriYear(days, 1448);
    assert.equal(y1448[0].id, 'hicriYilbasi');
    assert.equal(y1448[0].date, '2026-06-16');
    const last = y1448[y1448.length - 1];
    assert.deepEqual([last.id, last.part, last.date], ['kurbanBayrami', 4, '2027-05-19']);
    // İki miladi listeye yayılıyor ama yılın bütün dini günleri tam.
    assert.equal(y1448.length, 18);
    assert.equal(isHijriYearIncomplete(days, 1448), false);
    // 1449'un Berat'tan sonrası 2028 listesinde; henüz yayımlanmadı.
    assert.equal(isHijriYearIncomplete(days, 1449), true);
});

test('acilista etkin ya da siradaki gunun hicri yili; gecmis yillar sekmede yok', () => {
    assert.equal(defaultSpecialDaysHijriYear(days, at(2026, 9, 11)), 1448);
    assert.deepEqual(specialDaysHijriYears(days, at(2026, 9, 11)), [1448, 1449]);
    // Kurban Bayrami bitti, Hicri Yilbasi 1449 henuz gelmedi: sıradaki 1449'da.
    assert.equal(defaultSpecialDaysHijriYear(days, at(2027, 5, 25)), 1449);
    assert.deepEqual(specialDaysHijriYears(days, at(2027, 5, 25)), [1449]);
    // Bugun Kurban Bayraminin son gunu: hala 1448.
    assert.equal(defaultSpecialDaysHijriYear(days, at(2027, 5, 19, 20)), 1448);
    // Veri bittiyse son yil.
    assert.equal(defaultSpecialDaysHijriYear(days, at(2031, 1, 1)), 1449);
    assert.equal(defaultSpecialDaysHijriYear([], at(2031, 1, 1)), null);
    assert.deepEqual(specialDaysHijriYears([], at(2031, 1, 1)), []);
});

test('test onizlemesi: secilen gunun siradaki tarihinde aksam 21:00', () => {
    const now = at(2026, 9, 11, 10);
    const kadir = specialDayPreviewNow(days, 'kadir', now);
    assert.deepEqual([kadir.getFullYear(), kadir.getMonth() + 1, kadir.getDate(), kadir.getHours()], [2027, 3, 5, 21]);
    assert.equal(activeSpecialDay(days, kadir).id, 'kadir');
    // true: siradaki dini gun (Regaib, ayni gunku Uc Aylar'dan once gelir).
    const next = specialDayPreviewNow(days, true, now);
    assert.equal(activeSpecialDay(days, next).id, 'regaib');
    // Siradaki yoksa sonuncusu.
    const past = specialDayPreviewNow(days, 'mevlid', at(2031, 1, 1));
    assert.equal(past.getFullYear(), 2027);
    for (const bad of ['yok', false, null, undefined, 1]) assert.equal(specialDayPreviewNow(days, bad, now), null);
});
