...
let transientNoteDismiss = null;

/** Balonu söndürür ve bıraktığı dinleyiciyi toplar. */
function hideTransientNote() {
    clearTimeout(transientNoteTimer);
    document.getElementById('appTransientNote')?.classList.remove('app-note--in');
    if (transientNoteDismiss) {
        document.removeEventListener('pointerdown', transientNoteDismiss, true);
        transientNoteDismiss = null;
    }
}
 ...
 * Ekranın boş bir yerine dokunmak da söndürür: okuyan beklemek zorunda kalmasın.
 */
export function showTransientNote(message, ms = 1800) {
    ...
    clearTimeout(transientNoteTimer);
    transientNoteTimer = setTimeout(hideTransientNote, ms);
    // Bu dokunuşun kendisi değil, bir sonraki dokunuş söndürür: balon
    // gösterildiğinde pointerdown çoktan olmuş oluyor.
    if (transientNoteDismiss) {
        document.removeEventListener('pointerdown', transientNoteDismiss, true);
    }
    transientNoteDismiss = hideTransientNote;
    document.addEventListener('pointerdown', transientNoteDismiss, true);
}
