...
import {
    HATIM_CODE_ALPHABET,
    HATIM_CODE_LENGTH,
    HATIM_ACTIVE_LIMIT_PER_KIND,
    HATIM_ARCHIVE_LIMIT_PER_KIND,
    HATIM_FREEZE_AFTER_DAYS,
    applyJuzActionLocally,
    countActiveHatims,
    hatimCompletedAt,
    isHatimFrozen,
    tidyHatimArchive,
...
test('hatim sayısı kip başına ayrı kesilir', () => {
    ...
    // Devam edenler kendi kotasından kesilir.
    const many = Array.from({ length: 20 }, (_, i) => ({ id: `h_${i}`, name: `G${i}` }));
    assert.equal(sanitizeHatimGroups(many).length, HATIM_ACTIVE_LIMIT_PER_KIND);
    assert.deepEqual(sanitizeHatimGroups('dizi değil'), []);

    // Düz slice olsaydı iki kipi dolu tutan kullanıcı yarısını kaybederdi.
    const karisik = [
    ...
    assert.equal(temiz.length, HATIM_ACTIVE_LIMIT_PER_KIND * 2);
    assert.equal(countHatimsByKind(temiz, HATIM_KIND_PERSONAL), HATIM_ACTIVE_LIMIT_PER_KIND);
    assert.equal(countHatimsByKind(temiz, HATIM_KIND_SHARED), HATIM_ACTIVE_LIMIT_PER_KIND);
...
test('sınır kipler arasında paylaşılmaz', () => {
    const besKisisel = Array.from({ length: HATIM_ACTIVE_LIMIT_PER_KIND }, (_, i) => ({
        id: `p_${i}`,
...
test('ortak hatimin sunucu bilgisi depodan dönüşte korunur', () => {
    ...
    const [g] = sanitizeHatimGroups([
        {
            ...
            remote: {
                ...
                memberCount: 4,
                memberUids: ['u1', 'u2'],
                seenMembers: [{ uid: 'u1', name: 'Ömer' }]
            }
        }
    ]);
    // Üye listesi ve son bakılan hâl de kalmalı; yoksa üye noktaları her açılışta yanar.
    // Ad da kalmalı: ayrılan kişi listede adıyla gösteriliyor.
    assert.deepEqual(g.remote, {
        rev: 7,
        memberCount: 4,
        memberUids: ['u1', 'u2'],
        seenMembers: [{ uid: 'u1', name: 'Ömer' }]
    ...
    // Eski kayıtlar düz kimlik dizisiydi; okunmaya devam etmeli.
    const [eski] = sanitizeHatimGroups([
        {
            id: 'Xq3vT9bLm2Pz8KwR4nYc',
            kind: HATIM_KIND_SHARED,
            code: 'K7M2QP',
            remote: { rev: 1, memberCount: 2, memberUids: ['u1'], seenMembers: ['u1', 'u2'] }
        }
    ]);
    assert.deepEqual(eski.remote.seenMembers, [{ uid: 'u1', name: '' }, { uid: 'u2', name: '' }]);
...
test('bozuk sunucu bilgisi güvenli varsayılana düşer', () => {
    ...
    assert.deepEqual(g.remote, { rev: 0, memberCount: 1, memberUids: [] });
});

test('şerit kişisel ve paylaşımlı hatimdeki iki kimliği birlikte tanır', () => {
    // Kişisel hatimde cüz yerel kimliğe, paylaşımlıda Firebase uid'sine bağlı.
    // Tek kimlik verilseydi biri şeritten düşerdi.
    let kisisel = createHatimGroup({ kind: HATIM_KIND_PERSONAL, ownerId: ME });
    kisisel = claimJuz(kisisel, 7, ME).group;
    let ortak = createHatimGroup({ kind: HATIM_KIND_SHARED, ownerId: 'uid_x' });
    ortak = claimJuz(ortak, 7, 'uid_x').group;

    assert.equal(buildMemberJuzStrip([kisisel, ortak], ME)[6].pending, 1, 'tek kimlik yalnız birini görür');
    assert.equal(buildMemberJuzStrip([kisisel, ortak], [ME, 'uid_x'])[6].pending, 2);
    assert.equal(buildMemberJuzStrip([kisisel, ortak], [null, undefined])[6].state, JUZ_FREE);
});

/* ---------- Adın altındaki tarih satırı ---------- */

test('başlangıç tarihi bilinmiyorsa satır gösterilmez', () => {
    assert.equal(hatimTimeline({ createdAt: 0, juz: [] }), null);
    assert.equal(hatimTimeline({ juz: [] }), null);
    assert.equal(hatimTimeline(null), null);
});

test('gün sayısı takvim günüdür, saat farkı değil', () => {
    const g = createHatimGroup({ now: new Date(2026, 8, 10, 23, 0).getTime() });
    // Dün gece başlayan hatim, iki saat sonra bile "1 gündür" sayılır.
    assert.equal(hatimTimeline(g, new Date(2026, 8, 11, 1, 0).getTime()).days, 1);
    assert.equal(hatimTimeline(g, new Date(2026, 8, 10, 23, 30).getTime()).days, 0, 'aynı gün');
    assert.equal(hatimTimeline(g, new Date(2026, 8, 20, 9, 0).getTime()).days, 10);
    assert.equal(hatimTimeline(g, new Date(2026, 8, 20).getTime()).complete, false);
});

test('tamamlanan hatimde süre son cüzün bitişine kadar, bugüne kadar değil', () => {
    let g = createHatimGroup({ now: new Date(2026, 8, 1, 10, 0).getTime() });
    for (let n = 1; n <= 30; n += 1) {
        g = claimJuz(g, n, ME).group;
        const bitis = new Date(2026, 8, n < 30 ? 5 : 19, 12, 0).getTime();
        g = completeJuz(g, n, ME, { now: bitis }).group;
    }
    const satir = hatimTimeline(g, new Date(2026, 9, 1).getTime());
    assert.equal(satir.complete, true);
    assert.equal(satir.days, 18, '1 Eylül → 19 Eylül, 1 Ekim değil');
});

test('sınır yalnız devam eden hatimleri sayar', () => {
    // Biten hatim sunucudan okunmadığı için yer tutmaz; tutsaydı hatmi
    // bitirmenin bedeli onu silmek olurdu.
    const bitmis = Array.from({ length: HATIM_ACTIVE_LIMIT_PER_KIND }, (_, i) => ({
        id: `b_${i}`,
        kind: HATIM_KIND_SHARED,
        juz: Array.from({ length: 30 }, (_, i) => ({ n: i + 1, state: JUZ_DONE, by: 'u1', completedAt: 1000 }))
    }));
    assert.equal(isHatimLimitReached(bitmis, HATIM_KIND_SHARED), false);
    assert.equal(countActiveHatims(bitmis, HATIM_KIND_SHARED), 0);

    const devamEden = { id: `d`, kind: HATIM_KIND_SHARED, juz: [] };
    const karisik = [...bitmis, ...Array.from({ length: 5 }, () => devamEden)];
    assert.equal(isHatimLimitReached(karisik, HATIM_KIND_SHARED), true);
});

test('biten hatim birkaç gün sonra donar', () => {
    const bitis = Date.UTC(2026, 8, 1);
    const grup = { id: 'g', kind: HATIM_KIND_SHARED, juz: Array.from({ length: 30 }, (_, i) => ({ n: i + 1, state: JUZ_DONE, by: 'u1', completedAt: bitis })) };
    assert.equal(hatimCompletedAt(grup), bitis);
    assert.equal(
        isHatimFrozen(grup, bitis + (HATIM_FREEZE_AFTER_DAYS - 1) * 86400000),
        false,
        'süre dolmadan donmaz'
    );
    assert.equal(isHatimFrozen(grup, bitis + HATIM_FREEZE_AFTER_DAYS * 86400000), true);

    // Bitmemiş hatim donmaz; zaman damgası olmayan eski kayıt da donmaz.
    assert.equal(isHatimFrozen({ id: 'y', kind: HATIM_KIND_SHARED, juz: [] }, Date.now()), false);
    const damgasiz = { id: 'e', kind: HATIM_KIND_SHARED, juz: Array.from({ length: 30 }, (_, i) => ({ n: i + 1, state: JUZ_DONE, by: 'u1', completedAt: 0 })) };
    assert.equal(hatimCompletedAt(damgasiz), 0);
    assert.equal(isHatimFrozen(damgasiz, Date.now()), false);
});

test('arşiv tavanı aşılınca en eski biten düşer, donmuşun senkron defteri atılır', () => {
    const gun = 86400000;
    const simdi = Date.UTC(2026, 11, 1);
    const bitmisler = Array.from({ length: HATIM_ARCHIVE_LIMIT_PER_KIND + 2 }, (_, i) => ({
        id: `b_${i}`,
        kind: HATIM_KIND_SHARED,
        // i büyüdükçe daha yeni bitmiş
        juz: Array.from({ length: 30 }, (_, n) => ({
            n: n + 1,
            state: JUZ_DONE,
            by: 'u1',
            completedAt: simdi - (200 - i) * gun
        })),
        remote: { rev: 3, memberCount: 2, memberUids: [`u${i}`], seenMembers: [{ uid: `u${i}`, name: `A${i}` }] }
    }));
    const devam = { id: `d`, kind: HATIM_KIND_SHARED, juz: [], remote: { rev: 1, memberCount: 2, memberUids: [`u1`] } };

    const { groups, changed } = tidyHatimArchive([...bitmisler, devam], simdi);
    assert.equal(changed, true);
    assert.equal(groups.filter((g) => g.id.startsWith('b_')).length, HATIM_ARCHIVE_LIMIT_PER_KIND);
    assert.equal(groups.some((g) => g.id === 'b_0'), false, 'en eski düşer');
    assert.equal(groups.some((g) => g.id === 'b_1'), false);
    assert.equal(groups.some((g) => g.id === 'd'), true, 'devam eden korunur');

    // Donmuşların üye defteri atılır, devam edenlerinki durur.
    const donmus = groups.find((g) => g.id === 'b_5');
    assert.equal('memberUids' in donmus.remote, false);
    assert.equal('seenMembers' in donmus.remote, false);
    assert.equal(donmus.remote.rev, 3);
    assert.deepEqual(groups.find((g) => g.id === 'd').remote.memberUids, ['u1']);

    // İş bitince ikinci çağrı bir şey değiştirmemeli.
    assert.equal(tidyHatimArchive(groups, simdi).changed, false);
});

test('kırpmada devam eden hatim, eski arşiv uğruna düşmez', () => {
    // Listenin sonunda duran devam eden hatim, başta duran 30 bitmiş kayıt
    // yüzünden kesilirse kullanıcı çalışan hatmini kaybeder.
    const bitmisler = Array.from({ length: HATIM_ARCHIVE_LIMIT_PER_KIND + 4 }, (_, i) =>
        ({ id: 'b_${i}', kind: HATIM_KIND_SHARED, juz: Array.from({ length: 30 }, (_, n) => ({ n: n + 1, state: JUZ_DONE, by: 'u1', completedAt: 1000 + n })) })
    );
    const devam = Array.from({ length: HATIM_ACTIVE_LIMIT_PER_KIND }, (_, i) => ({
        id: `d_${i}`,
        kind: HATIM_KIND_SHARED
    }));
    const temiz = sanitizeHatimGroups([...bitmisler, ...devam]);

    assert.equal(countActiveHatims(temiz, HATIM_KIND_SHARED), HATIM_ACTIVE_LIMIT_PER_KIND);
    assert.equal(
        temiz.filter((g) => isHatimComplete(g)).length,
        HATIM_ARCHIVE_LIMIT_PER_KIND,
        'arşiv kendi tavanında kesilir'
    );
    assert.ok(temiz.some((g) => g.id === 'd_4'), 'sondaki devam eden korunur');
});

test('depoda arşiv taşmışsa en eski bitenler düşer, yeniler kalır', () => {
    // Sıraya göre kesseydi depoda 32 kayıt varken en son biten hatim
    // açılışta silinir, aylar önceki duruyor olurdu.
    const bitmis = (id, at) => ({
        id,
        kind: HATIM_KIND_SHARED,
        juz: Array.from({ length: 30 }, (_, n) => ({
    ...
    // Dizide eskiler önce: 0 en eski, sonuncusu en yeni.
    const hepsi = Array.from({ length: HATIM_ARCHIVE_LIMIT_PER_KIND + 2 }, (_, i) =>
        bitmis(`b_${i}`, 1000 + i)
    );
    const temiz = sanitizeHatimGroups(hepsi);
    assert.equal(temiz.length, HATIM_ARCHIVE_LIMIT_PER_KIND);
    assert.equal(temiz.some((g) => g.id === 'b_0'), false, 'en eski düşer');
    assert.equal(temiz.some((g) => g.id === 'b_1'), false);
    assert.equal(
        temiz.some((g) => g.id === `b_${HATIM_ARCHIVE_LIMIT_PER_KIND + 1}`),
        true,
        'en son biten kalır'
    );
});

test('iyimser güncelleme: her işlemin yerel karşılığı', () => {
    const bos = createHatimGroup({ kind: HATIM_KIND_SHARED, ownerId: 'u0', name: 'G' });

    const alindi = applyJuzActionLocally(bos, 5, 'claim', 'u1', { memberName: 'Ali', now: 100 });
    assert.equal(alindi.juz[4].state, JUZ_CLAIMED);
    assert.equal(alindi.juz[4].by, 'u1');
    assert.equal(alindi.juz[4].byName, 'Ali');
    assert.equal(bos.juz[4].state, JUZ_FREE, 'önceki hâl değişmez — geri alma bunun üstüne kurulu');

    const bitti = applyJuzActionLocally(alindi, 5, 'complete', 'u1', { now: 200 });
    assert.equal(bitti.juz[4].state, JUZ_DONE);
    assert.equal(bitti.juz[4].completedAt, 200);

    const geri = applyJuzActionLocally(bitti, 5, 'uncomplete', 'u1');
    assert.equal(geri.juz[4].state, JUZ_CLAIMED);

    const birakti = applyJuzActionLocally(geri, 5, 'release', 'u1');
    assert.equal(birakti.juz[4].state, JUZ_FREE);

    const zorla = applyJuzActionLocally(alindi, 5, 'forceRelease', 'u0');
    assert.equal(zorla.juz[4].state, JUZ_FREE, 'yönetici başkasının cüzünü boşaltır');
});

test('iyimser güncelleme: yerelde bile geçersizse yapılmaz', () => {
    const bos = createHatimGroup({ kind: HATIM_KIND_SHARED, ownerId: 'u0', name: 'G' });
    const alindi = applyJuzActionLocally(bos, 5, 'claim', 'u1', { memberName: 'Ali' });
    // Önbellekte başkasında görünen cüz: karar sunucunun, ekrana bir şey basılmaz.
    assert.equal(applyJuzActionLocally(alindi, 5, 'claim', 'u2', { memberName: 'Veli' }), null);
    assert.equal(applyJuzActionLocally(alindi, 5, 'complete', 'u2'), null);
    assert.equal(applyJuzActionLocally(bos, 5, 'release', 'u1'), null);
    assert.equal(applyJuzActionLocally(bos, 99, 'claim', 'u1'), null);
    assert.equal(applyJuzActionLocally(bos, 5, 'uydurma', 'u1'), null);
});
