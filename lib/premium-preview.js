...
/**
 * Önizleme modunda entitlement'ı ekrana göre ayarla (kaydedilen gerçek aboneliği ezmez kalıcı olarak
 * yalnızca bu oturumda UI testi için).
 * @param {PremiumPreviewMode|null} mode
 * @param {{ premium?: boolean }} entitlementsRef
 */
export function applyPremiumPreviewEntitlement(mode, entitlementsRef) {
    if (!mode || !entitlementsRef) return;
    if (mode === 'active') entitlementsRef.premium = true;
    else entitlementsRef.premium = false;
}
