...
test('public/library-overrides.json dosyası geçerlidir ve lib_21 maddesini içerir', async () => {
    const fs = await import('node:fs');
    const raw = JSON.parse(fs.readFileSync(new URL('../public/library-overrides.json', import.meta.url), 'utf8'));
    const norm = normalizeLibraryOverrides(raw);
    assert.ok(norm);
    assert.ok(norm.items.lib_21);
    assert.equal(norm.items.lib_21.byLayer.tr.target, 100);
    assert.ok(norm.items.lib_21.prev.fazilet.length >= 2);
    assert.ok(norm.items.lib_21.prev.name.length >= 5);
});
