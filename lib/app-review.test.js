...
test('bekleme süresi makul aralıkta (kendi ayağımıza sıkmayalım)', () => {
    // Asıl fren Google kotası; API pencere açıldı mı bildirmediği için
    // uzun bekleme fırsat yakar. 30-90 gün dışına çıkarsa kasıtlı olmalı.
    assert.ok(
        MIN_DAYS_BETWEEN_ASKS >= 30 && MIN_DAYS_BETWEEN_ASKS <= 90,
        `MIN_DAYS_BETWEEN_ASKS=${MIN_DAYS_BETWEEN_ASKS} beklenen 30-90 aralığı dışında`
    );
});
