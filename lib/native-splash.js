...
import { reapplyNativeStatusBarThemeAfterSplash } from '../status-bar-theme.js';
...
export async function hideNativeSplashWhenReady(shownAtMs = performance.now()) {
    ...
    try {
        ...
        // Splash çıkışı durum çubuğu ikonlarını XML temasına sıfırlıyor (bkz. status-bar-theme.js)
        reapplyNativeStatusBarThemeAfterSplash();
    } catch (err) {
