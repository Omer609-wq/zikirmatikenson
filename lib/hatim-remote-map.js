...
export function groupFromRemote(hatimId, hatimData, juzByN, previous = null, memberUids = null) {
    ...
    const seen = normalizeMemberUids(previous?.remote?.seenMembers);
    return {
        id: hatimId,
...
export function memberDiff(group) {
    ...
    const seen = normalizeMemberUids(group?.remote?.seenMembers);
    if (!seen) return { joined: [], left: [], firstLook: true };
    return {
        joined: current.filter((uid) => !seen.includes(uid)),
        left: seen.filter((uid) => !current.includes(uid)),
        firstLook: false
    };
}

/** Listeye bakıldı: noktalar söner. `uids` verilirse o an okunan liste temel alınır. */
export function withSeenMembers(group, uids = null) {
    if (!isRemoteHatim(group)) return group;
    const current = normalizeMemberUids(uids) || normalizeMemberUids(group.remote.memberUids) || [];
    return { ...group, remote: { ...group.remote, memberUids: current, seenMembers: current } };
}
