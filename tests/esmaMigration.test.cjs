...
function loadAppIntoContext(context) {
    ...
    source = source.replace(
        /^import[\s\S]+?from '\.\/esma-fazilet\.js';\n/,
...
const runDragReorderNudge = async () => {};
...
assert.equal(byId.get('z_e_0').name, 'Yâ Allah');
assert.equal(byId.get('z_e_0').count, 0);
assert.equal(byId.get('z_e_1').name, 'Yâ Rahman');
assert.equal(byId.get('z_e_1').count, 7);
assert.equal(byId.get('z_e_2').name, 'Yâ Rahîm');
assert.equal(byId.get('z_e_2').count, 11);
assert.equal(byId.get('z_e_2').favorite, true);
assert.equal(byId.get('z_e_20').name, 'Yâ Kâbıd');
assert.equal(byId.get('z_e_20').count, 18);
assert.equal(byId.get('z_e_20').lastClicked, 400);

assert.equal(migratedHistory.z_e_1, 2);
assert.equal(migratedHistory.z_e_2, 4);
assert.equal(migratedHistory.z_e_20, 14);
